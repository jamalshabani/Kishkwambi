...
const StepInterchangeDocumentPhoto = ({ onBack, containerData }) => {
    ...
    // Camera overlay dimensions for interchange document photos
    const screenWidth = Dimensions.get('window').width;
    const screenHeight = Dimensions.get('window').height;
    const interchangeFrameWidth = screenWidth * 0.80; // 80% of screen width
    const interchangeFrameHeight = screenHeight * 0.70; // 70% of screen height for document
    
    // Calculate the center position for the interchange document frame
    const centerX = (screenWidth - interchangeFrameWidth) / 2;
    const centerY = 80; // Position from top

    // Function to calculate crop area based on interchange document frame dimensions
    const calculateCropArea = (imageWidth, imageHeight) => {
        ...
        let imageOverlayWidth = interchangeFrameWidth * scale;
        let imageOverlayHeight = interchangeFrameHeight * scale;
    ...
    // Function to crop image to interchange document frame area
    const cropImageToInterchangeFrame = async (imageUri) => {
        try {
    ...
    const takePicture = async () => {
        ...
        try {
            ...
            if (photo?.uri) {
                // Crop the image to the interchange document frame area
                const croppedImage = await cropImageToInterchangeFrame(photo.uri);
                
                console.log('📸 Interchange document photo taken and cropped:', croppedImage);
    ...
    return (
        <SafeAreaView style={cn(`flex-1 ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`)} edges={['top']}>
            ...
            <View style={cn('flex-1')}>
                ...
                {/* Custom Mask Overlay - darkens everything except the interchange document frame */}
                <View style={cn('absolute inset-0')} pointerEvents="box-none">
                    ...
                    <View 
                        style={{
                            position: 'absolute',
                            top: centerY,
                            left: centerX + interchangeFrameWidth,
                            width: screenWidth - (centerX + interchangeFrameWidth),
                            height: screenHeight - centerY,
                            backgroundColor: 'black'
                        }}
                    />
                    ...
                    <View 
                        style={{
                            position: 'absolute',
                            top: centerY + interchangeFrameHeight,
                            left: 0,
                            width: screenWidth,
                            height: screenHeight - (centerY + interchangeFrameHeight),
                            backgroundColor: 'black'
                        }}
                    />
                    
                    {/* Interchange Document Frame Guide - green outline with 80% opacity */}
                    <View 
                        style={{
                            position: 'absolute',
                            top: centerY,
                            left: centerX,
                            width: interchangeFrameWidth,
                            height: interchangeFrameHeight,
                            borderWidth: 2,
                ...
                <View style={cn('absolute top-4 left-4 right-4 items-center')} pointerEvents="none">
                    <View style={cn('pt-2 rounded-lg')}>
                        <Text style={cn('text-white text-center text-lg font-semibold')}>
                            Capture the Interchange Document
                        </Text>
                    </View>
                </View>
...
export default StepInterchangeDocumentPhoto;
